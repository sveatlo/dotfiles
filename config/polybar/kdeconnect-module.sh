...
COLOR_BATTERY_70='#ccc'         # Battery >= 80
COLOR_BATTERY_50='#aaa'         # Battery >= 70
COLOR_BATTERY_30='#888'         # Battery >= 60
...
show_devices (){
    ...
    for device in $(qdbus --literal org.kde.kdeconnect /modules/kdeconnect org.kde.kdeconnect.daemon.devices); do
        ...
        then
            ...
            devices+="%{A1:$SCRIPT_FULLPATH -n '$devicename' -i $deviceid -b $battery -m:}$icon%{A}$SEPERATOR"
        ...
        then
            ...
            devices+="%{A1:$SCRIPT_FULLPATH -n $devicename -i $deviceid -p:}$icon%{A}$SEPERATOR"
...
show_menu () {
    menu="$($ROFI_BASE -p "$DEV_NAME" -lines 8 <<< "Battery: $DEV_BATTERY%|Ping|Find Device|Send File|Send SMS|Browse Files|Launch SMS app|Settings|Unpair")"
    case "$menu" in
        ...
        *'Send SMS')
            phone_number=$(echo "" | rofi -lines 0 -dmenu -p "Enter phone number")
            message=$(echo "" | rofi -lines 0 -dmenu -p "Enter message")
            qdbus org.kde.kdeconnect "/modules/kdeconnect/devices/$DEV_ID/sms" org.kde.kdeconnect.device.sms.sendSms "$phone_number" "$message"
            # qdbus org.kde.kdeconnect "/modules/kdeconnect/devices/$DEV_ID/sms" org.kde.kdeconnect.device.sms.launchApp
            ;;
        *'Browse Files')
...
show_pmenu () {
    menu="$($ROFI_BASE -p "$DEV_NAME" -lines 1<<<"Pair Device")"
                case "$menu" in
...
show_pmenu2 () {
    menu="$($ROFI_BASE -p "$1 has sent a pairing request" -lines 2 <<< "Accept|Reject")"
                case "$menu" in
